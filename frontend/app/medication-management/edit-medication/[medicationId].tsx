...
import { updateMedicationForSelf } from '@/lib/medication';
import { Ionicons } from '@expo/vector-icons';
import { LinearGradient } from 'expo-linear-gradient';

import { getMedicationForSelfWithSchedule } from '@/lib/medication';
...
export default function EditMedicationScreen() {
  ...
  useEffect(() => {
    const fetchMedication = async () => {
      try {
        const data = await getMedicationForSelfWithSchedule(medicationId);
        setName(data.name);
  ...
  if (userRole === 'main_user') {
    router.replace('/(drawer)/(tabs-amain)/medication-management');
  } else if (userRole === 'dependent_user') {
    router.replace('/(drawer)/(tabs-dependent)/medication-management');
  }
};
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Navigate back to the medication management screen

      const userRole = await fetchUserRole();
      console.log('User role after fetching:', userRole);
      if (userRole === 'main_user') {
      router.push('/(drawer)/(tabs-amain)/medication-management');
      }else if (userRole === 'dependent_user') {
      router.push('/(drawer)/(tabs-dependent)/medication-management');
      }
    } catch (error) {
      ...
      <View style={styles.container}>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.header}>
            ...
            <Text style={[styles.headerTitle, { color: 'white', fontWeight: '700' }]}>
              Edit Medication
            </Text>
          </View>
        </View>
        ...
        <ScrollView style={styles.scrollContainer}>
          <View style={styles.innerContainer}>
            <Text style={[styles.sectionTitle, { color: '#333', fontWeight: 'bold' }]}>Medication Name</Text>
            <TextInput
            ...
            <View style={styles.section}>
              <Text style={[styles.sectionTitle, { color: '#333', fontWeight: 'bold' }]}>Medication Type</Text>
                ...
                {medicationTypeOptions.map((option) => (
                  ...
                  >
                    <Text
                      style={[
                        styles.optionText,
                        { color: medicationType === option.value ? 'white' : '#333' }
                      ]}
                    >
                      {option.label}
                    </Text>
            ...
            <View style={styles.section}>
              <Text style={[styles.sectionTitle, { color: '#333', fontWeight: 'bold' }]}>Medication Form</Text>
              <View style={styles.optionsContainer}>
                {medicationFormOptions.map((option) => (
                  ...
                  >
                    <Text
                      style={[
                        styles.optionText,
                        { color: medicationForm === option.value ? 'white' : '#333' }
                      ]}
                    >
                      {option.label}
                    </Text>
            ...
            <Text style={[styles.sectionTitle, { color: '#333', fontWeight: 'bold' }]}>Dosage</Text>
            <TextInput
            ...
            <Text style={[styles.sectionTitle, { color: '#333', fontWeight: 'bold' }]}>Stock Quantity</Text>
            <TextInput
            ...
            <View style={styles.section}>
              <Text style={[styles.sectionTitle, { color: '#333', fontWeight: 'bold' }]}>Days to Remind</Text>
                  ...
                  >
                    <Text
                      style={[
                        ...
                        { color: daysOfWeek.includes(day.value) ? 'white' : '#333' }
                      ]}
                    >
                      {day.label}
                    </Text>
            ...
            <View style={styles.section}>
              <Text style={[styles.sectionTitle, { color: '#333', fontWeight: 'bold' }]}>
                Reminder Times ({reminderTimes.length})
              </Text>
              {reminderTimes.map((time, index) => (
                <View key={index} style={styles.timeContainer}>
                  <Text style={[styles.timeText, { color: '#333' }]}>
                    {index + 1}. {time}
                  </Text>
               ...
               <TouchableOpacity
                          ...
                          >
                            <Text style={[styles.addTimeButtonText, { color: 'white', fontWeight: 'bold' }]}>
                              + Add Reminder Time
                            </Text>
                          </TouchableOpacity>
            ...
            <TouchableOpacity style={styles.submitButton} onPress={handleSubmit}>
              <Text style={[styles.submitText, { color: 'white', fontWeight: 'bold' }]}>Save Changes</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      </View>
