...
export default function ScheduleScreen() {
  ...
  const renderCalendar = () => {
    ...
    for (let i = 0; i < firstDay; i++) {
      week.push(<View key={`empty-start-${selectedDate.getMonth()}-${i}`} style={styles.calendarDay} />);
    }
    ...
    for (let day = 1; day <= days; day++) {
      ...
      week.push(
        <TouchableOpacity
          key={`day-${date.toISOString()}`}  // Use full date string to ensure unique key
          style={[
            styles.calendarDay,
            isSelectedDate && { backgroundColor: '#1a8e2d15' },
            isToday && !isSelectedDate && { backgroundColor: 'transparent' },
          ]}
          onPress={() => {
        ...
        >
          <Text
            style={[
              styles.dayText,
              isSelectedDate && { color: '#1a8e2d', fontWeight: '600' },
              isToday && !isSelectedDate && styles.dayText,
            ]}
          >
            {day}
          </Text>
      ...
      if ((firstDay + day) % 7 === 0 || day === days) {
        while (week.length < 7) {
          week.push(
            <View key={`empty-end-${selectedDate.getMonth()}-${week.length}`} style={styles.calendarDay} />
          );
        }
        calendar.push(<View key={`week-${selectedDate.getMonth()}-${day}`} style={styles.calendarWeek}>{week}</View>);
        week = [];
      }
    }
  ...
  const renderMedicationForDate = () => {
    ...
    return currentMedications.map((medication, index) => {
      const status = medication.medicationStatus;
      const reminderTime = new Date(medication.reminderTime);
      const currentTime = new Date();

      const uniqueKey = `${medication.id || index}_${medication.reminderTime}`;
      ...
      return (
        <View style={styles.medicationCard} key={uniqueKey}>
          ...
          {status === 'taken' ? (
            ...
            <TouchableOpacity
              style={[
                ...
                { 
                  backgroundColor: status === 'late' ? '#1a8e2d' : '#1a8e2d',
  ...
  {status === 'late' || status === 'taken' ? 'Taken' : 'Not Taken'}
</Text>
            </TouchableOpacity>
          )}
        </View>
