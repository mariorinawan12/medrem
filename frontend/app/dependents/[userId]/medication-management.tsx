...
import { getMedicationsForDependent, deleteMedicationForDependent } from '@/lib/medication-dependent';
...
export default function DependentMedicationScreen() {
  ...
  const fetchMedications = async () => {
    try {
      const data = await getMedicationsForDependent(userId);
      setMedications(data);
  ...
  const handleDelete = async (medicationId: string) => {
    try {
      await deleteMedicationForDependent(userId.toString(), medicationId);
          ...
          <View style={styles.menu}>
            <TouchableOpacity
              onPress={() => {
                ...
                router.push({
                  pathname: '/dependents/[userId]/edit-medication/[medicationId]',
                  params: { 
  ...
  onPress={() => {
    // Tembak langsung ke halaman medication management si user ini
    router.replace({
...
>
  <Ionicons name="chevron-back" size={28} color="white" />
          ...
          <Text style={styles.headerTitle}>{fullName}'s Medications</Text>
        </View>
        ...
        <View style={styles.buttonOuterContainer}>
          <TouchableOpacity
            style={styles.button}
            onPress={() => router.push({
              pathname: '/dependents/[userId]/add-medication-new',
              params: { 
                userId: userId.toString(), 
                fullName: fullName.toString() 
              },
...
const styles = StyleSheet.create({
  ...
  headerGradient: {
    ...
    left: 0,
    right: 0,
    height: 120, // Increased height to accommodate back button
  },
  header: {
    flexDirection: 'row',
    alignItems: 'center',
