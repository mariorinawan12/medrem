...
import { getActiveDatesDependent, getMedicationsForDateDependent } from '@/lib/medication-dependent';
...
export default function ScheduleScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const meds = await getActiveDatesDependent(userId);
      setMedications(meds);
  ...
  const fetchMedicationsForDate = async (date: Date) => {
    ...
    try {
      const medsForDate = await getMedicationsForDateDependent(dateStr, userId);
  ...
  const renderMedicationForDate = () => {
    ...
    return currentMedications.map((medication) => {
      ...
      const isPending = status === 'not-taken';
